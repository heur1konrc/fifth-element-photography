...
def run_import_endpoint():
    """Execute the new import script and return results"""
    try:
        # Change to the app directory
        app_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Run the NEW import script
        result = subprocess.run(
            ['python3', 'import_all_products_new.py'],
            cwd=app_dir,
            capture_output=True,
            text=True,
...
def check_import_status():
    """Check if products have been imported with new structure"""
    try:
        import sqlite3
...
def register_import_routes(app):
    """Register admin import routes with new structure"""
    
    @app.route('/admin/import-lumaprints-products', methods=['POST'])
    def admin_import_products():
        """Run the Lumaprints product import with new structure"""

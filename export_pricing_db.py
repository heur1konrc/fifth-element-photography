"""
Export pricing database structure and data to JSON
Run this on Railway to analyze the production database
"""
...
def export_database():
    ...
    export_data['samples'] = {}
    cursor.execute("SELECT DISTINCT category_id FROM products")
    for row in cursor.fetchall():
        ...
        cursor.execute("""
            ...
            LIMIT 3
        """, (cat_id,))
        export_data['samples'][f'category_{cat_id}'] = [dict(r) for r in cursor.fetchall()]
    
    conn.close()
    
    # Save to file
    output_file = '/tmp/pricing_db_export.json'
    with open(output_file, 'w') as f:
        json.dump(export_data, f, indent=2)
    
    print(f"Database exported to {output_file}")
    print(f"Total products: {export_data['statistics']['total_products']}")
    print(f"Total categories: {export_data['statistics']['total_categories']}")
    print(f"Global markup: {export_data['settings'].get('global_markup_percentage', 'Not set')}")
